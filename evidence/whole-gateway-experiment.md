...
GatewayのMXC内起動とGateway内直接filesystem処理の境界は成立しました。ただし、Core successの必須条件であるモデル接続、agent session、structured `exec` tool callは成立していません。原因は、安定版OpenClawが利用するCodex app-serverがMXC AppContainer-DACL内で`CODEX_HOME`をcanonicalizeするとWindows error 5（access denied）になることです。失敗はモデル認証・outbound接続・exec実行より前の層です。
